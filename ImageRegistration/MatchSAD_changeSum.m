...
%     [m,n] = size(paddingsrc);
    ...
    for i=1:m-M+1         %子图选取，每次滑动一个像�? 
        for j=1:n-N+1  
%     for i=1:1        %子图选取，每次滑动一个像�? 
%          for j=1:1 
...
%              dst(i,j)= dst(i,j) + sum(abs(double(temp)- double(mask)),'all')/nonzerolen;   
            ...
            %误差平方和算法（SSD�?
    %       dst(i,j)=dst(i,j)+sum(sum((temp-mask).^2));%.^：矩阵中每个元素的平�? 
        end  
    end  

    % m=min(dst);%找出矩阵dst中每列中的最小元素，构成行向量m
    % mm=min(m);%进一步找出m中的�?��元素mm，当然也就是矩阵dst中的�?��元素
    value=min(min(dst));
    [r,c]=find(dst==value);%返回�?��值在dst（同样也在src）中的行列号 r：行 c：列  
    value = value/(M*N);
%     dis(kk,2)=value;
    ...
    if ifshow
        ...
        imshow(src);  
        hold on;  
        rectangle('position',[x,y,N-1,M-1],'edgecolor','r'); %[x坐标,y坐标,�?高] 
        hold off;title('匹配结果');  
    end
end